...
PlayStateClass = GameStateClass.extend({
	...
	free_dig_at : function (freex, freey) {
	...
	remove_stuck_mines : function () {
		for(var y = 0; y < this.grid_h; y++) {
            		for(var x = 0; x < this.grid_w; x++) {
				if (this.solver_cover[x][y] != 1) continue;	// 0 == dug solved, 1 == covered , 2 == flag
				var fringe = false;
				if (x > 0 && this.solver_cover[x - 1][y] != 1) fringe = true;
				if (y > 0 && this.solver_cover[x][y - 1] != 1) fringe = true;
				if (x < this.grid_w - 1 && this.solver_cover[x + 1][y] != 1) fringe = true;
				if (y < this.grid_h - 1 && this.solver_cover[x][y + 1] != 1) fringe = true;

				if (x > 0 && y > 0 && this.solver_cover[x - 1][y - 1] != 1) fringe = true;
				if (x > 0 && y < this.grid_h - 1 && this.solver_cover[x - 1][y + 1] != 1) fringe = true;
				if (x < this.grid_w - 1 && y > 0 && this.solver_cover[x + 1][y - 1] != 1) fringe = true;
				if (x < this.grid_w - 1 && y < this.grid_h - 1 && this.solver_cover[x + 1][y + 1] != 1) fringe = true;

				if (fringe == false) continue;
		...
		while (changes_ > 0 && loops_ < 999) {
			...
			for(var y = 0; y < this.grid_h; y++) {
				...
				for (var r = 0; r < this.blocks[this.tiles[x][y]].x_in_range.length; r++) {
					...
					// obvious safe:
					if (this.solver_cover[x_r][y_r] == 1 &&
				...
				var exhausted_ = this.set_next_temp_flag_config(x,y);
				var loops_ = 0;
				while (exhausted_ == 0 && loops < 99) {
			...
			if (progress == this.grid_w*this.grid_h) return true;
			if (changes_ == 0) return false;
		} // while
	},
	...
	draw_background : function () {
		...
		for(var y = 0; y < this.grid_h; y++) {
            		for(var x = 0; x < this.grid_w; x++) {
				var top_y = Math.max((y+0.5)*this.tile_size - 25, 2);
				var bottom_y = Math.min((y+0.5)*this.tile_size +25, this.tile_size*this.grid_h - 2);
				var left_x = Math.max((x+0.5)*this.tile_size - 25, 2);
				var right_x =  Math.min((x+0.5)*this.tile_size +25, this.tile_size*this.grid_w - 2);
				var sq_;
...
GenerateRandStateClass = GameStateClass.extend({
	...
	init: function(engine, play_state){
		...
		this.play_state.level_w = 12;
		this.play_state.level_h = 12;
...
DuringGameStateClass = GameStateClass.extend({
	...
	screen_resized: function () {
		...
		if (this.show_level_text == true) {
			g_level_text_1.update_pos(0.25*this.play_state.tile_size, 8*this.play_state.tile_size,9999,999);
			g_level_text_2.update_pos(0.25*this.play_state.tile_size, 10.5*this.play_state.tile_size,9999,999);
		}
