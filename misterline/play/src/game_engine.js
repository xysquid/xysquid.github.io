...
var gettheapp_url = 'http://www.zblip.com/misterline';
...
Types = {
		...
		Events: {
			...
			LEFT_MOUSE_DOWN: 100,
			RIGHT_MOUSE_DOWN: 101,

			MENU_POPPED_DOWN: 200,	

			UNLOCK_ALL_LEVELS: 300,
...
MenuItems = [
	...
	//[0, "Game"],
	[1, Types.Events.MENU, "MENU","home_icon.png",],
	[1, Types.Events.GOTO_LEVELS, "LEVELS","home_icon.png",],
	//[1, Types.Events.NEW_GAME, "RANDOM","home_icon.png",],
	[1, Types.Events.GOTO_CHALLENGE, "CHAPTERS","home_icon.png",],
...
if (using_cocoon_js == false) {
	if (g_servermode == true) MenuItems.push([1, Types.Events.GOTO_EDITOR, g_get_text("LEVEL EDITOR"),"home_icon.png"]);
...
MenuItems.push([1, Types.Events.WEB_LINK, "GET THE APP","ic_list_white_24dp_2x.png",gettheapp_url]);
...
MenuItems.push([0, "SETTINGS"]);
//MenuItems.push([1, Types.Events.HIDE_ONES, "HIDE ONES","redflag.png",]);
...
MenuItems.push([1, Types.Events.MUSIC_ONOFF, "TOGGLE MUSIC","sound_on_icon.png","sound_off_icon.png"]);
...
//MenuItems.push([1, Types.Events.UNLOCK_ALL_LEVELS, "UNLOCK ALL LEVELS","sound_on_icon.png","sound_off_icon.png"]);
MenuItems.push([1, Types.Events.CURSOR_ONOFF, "CURSOR ON","sound_on_icon.png","sound_off_icon.png"]);
...
//MenuItems.push([1, Types.Events.WEB_LINK, "CREDITS","ic_list_white_24dp_2x.png","http://www.zblip.com/braincross/credits"]);
...
BlipFrogMenuClass = Class.extend({
	...
	setup: function() {
		...
		this.engine_fb_rect = new SquareClass(0,0,29*4,29*4,Types.Layer.POP_MENU,0x5BB9BF,true);
		...
		*/
			...
			if (using_phaser == true && MenuItems[i][0] == 0) {
				...
				graphics_obj.beginFill(0x5BB9BF);
				graphics_obj.lineStyle(0, 0x5BB9BF, 0);
				if (i != 0) {
				...
				outline.lineStyle(4,0x222222);	// 0xF8BC28
				outline.moveTo(this.menu_width - 2, 0);
				outline.lineTo(this.menu_width - 2, 999);
				options_menu_group.add(outline);
			}
			...
			if (MenuItems[i][0] == 0) {
				...
				this.menu_texts[i].set_colour("#ffffff");		// bl
	...
	show_all_menu_text: function() {
		for (var i = 0; i < this.sprites_buttons.length; i++) {
			...
			if (MenuItems[i][0] == 0) {
				...
				this.menu_texts[i].set_colour("#ffffff");		// black b4 outline
			}
	...
	handle_events: function(x, y, event_type) {
		...
		if (event_type == Types.Events.MOUSE_UP && 
			...
			//alert('pop down click -> false');
			//return;
			//event_type = Types.Events.MENU_POPPED_DOWN;
			x = 99999;
			y = 99999;
		} 


		if (this.pop_down_click == true) {
		...
		//if (event_type == Types.Events.MOUSE_DOWN) console.log(' Types.Events.MOUSE_DOWN');
	...
	handle_menu_event: function(x,y,event_type) {
		...
		if (MenuItems[menu_i][1] == Types.Events.NEW_GAME) {
		...
		}  else if (MenuItems[menu_i][1] == Types.Events.MENU) {

			this.game_engine.handle_events(0, 0, Types.Events.MENU);
		...
		} else if (MenuItems[menu_i][1] == Types.Events.GOTO_CHALLENGE) {

			this.game_engine.handle_events(0, 0, Types.Events.GOTO_CHALLENGE);
		...
		} else if (MenuItems[menu_i][1] == Types.Events.CURSOR_ONOFF) {
			//gSM.togglemute();
			if (g_cursor_on) {
				g_cursor_on = false;
				this.menu_texts[menu_i].change_text("CURSOR" + g_texts[language]["OFF"]);
			} else {
				g_cursor_on = true;
				this.menu_texts[menu_i].change_text("CURSOR" + g_texts[language]["ON"]);
			}

			

		} else if (MenuItems[menu_i][1] == Types.Events.UNLOCK_ALL_LEVELS) {
			unlock_all_levels();
			...
			//this.pop_down();
		} else if (MenuItems[menu_i][1] == Types.Events.HIDE_ONES) {
			g_hide_ones_on_grid = -g_hide_ones_on_grid;
			

			if (g_hide_ones_on_grid == 1) {
				this.menu_texts[menu_i].set_colour("#ffffff");
				g_nono_tile_sprites[11] = "bluetile.png";
				g_nono_tile_sprites_flag[11] = "redtile.png";
			} else {
				this.menu_texts[menu_i].set_colour("#000000");
				g_nono_tile_sprites[11] = "bluetiletut.png";
				g_nono_tile_sprites_flag[11] = "redtiletut.png";
			}

			
			g_should_redraw_grid = true;					
...
GameEngineClass = Class.extend({
	...
	handle_events: function(x, y, event_type) {
		...
		if (event_type == Types.Events.NEW_GAME) {
		...
		} else if (event_type == Types.Events.MENU) {
			console.log('Types.Events.MENU');
			while(this.state_stack.length > 2) {
				var state_ = this.state_stack.pop();
				state_.cleanup();
			}
			...
			var current_level = this.state_stack[1].current_level;

			while(this.state_stack.length > 2) {
				var state_ = this.state_stack.pop();
				state_.cleanup();
			}
			//this.state_stack[1].current_world = 0;
			
			this.push_state(new NewOverworldStateClass(this, this.state_stack[1]));
			this.state_stack[2].pagenum = Math.floor(current_level / 30);
			this.state_stack[2].load_page(this.state_stack[2].pagenum);
			//this.push_state(new ChallengeLevelsSelectHardness(this, this.state_stack[1]));

		} else if (event_type == Types.Events.GOTO_CHALLENGE) {
