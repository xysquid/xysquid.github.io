...
setup_pixi = function () {
	// create a renderer instance.
		...
		stage = new PIXI.Stage(0x8C8ACC);	// 8C8ACC(blue)...1F1129
		...
		renderer.backgroundColor = 0x8C8ACC;//0x1F1129
...
set_pixi_bg_colour = function (new_colour) {
	renderer.backgroundColor = new_colour;
...
animate = function (timestamp) {
	...
	// LEFT 37 W 65 // RIGHT 39 D 68	// UP 38 W 87// DOWN 40 S 83
	    if(key_down) {
				if(keyState[37] || keyState[65] ) gBlipFrogMenu.handle_events(mouse['x'],mouse['y'],Types.Events.KEY_LEFT);
				else if(keyState[39]|| keyState[68]) gBlipFrogMenu.handle_events(mouse['x'],mouse['y'],Types.Events.KEY_RIGHT);
				else if(keyState[38] || keyState[87]) gBlipFrogMenu.handle_events(mouse['x'],mouse['y'],Types.Events.KEY_UP);
				else if(keyState[40] || keyState[83]) gBlipFrogMenu.handle_events(mouse['x'],mouse['y'],Types.Events.KEY_DOWN);   
				else if(keyState[32]) gBlipFrogMenu.handle_events(mouse['x'],mouse['y'],Types.Events.KEY_SPACEBAR);   
	 ...
	 if(key_up) {
			key_up = false;
			gBlipFrogMenu.handle_events(mouse['x'],mouse['y'],Types.Events.KEY_RELEASE);
	}
