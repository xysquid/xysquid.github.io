...
SquareClass = Class.extend({
	...
	new_graphics : function(x, y, xx, yy,layer, colour, filled) {
		...
		if (using_phaser == true) {
			...
			this.pixisprite = new PIXI.Graphics();
			if (filled == true) this.pixisprite.beginFill(colour);
			if (filled == false) this.pixisprite.lineStyle( 6 , colour);
	...
	update_size : function(x, y, w, h) {
		...
		if (using_phaser == true) {
			...
			this.pixisprite.scale.x = w;
			this.pixisprite.scale.y = h;
		}
...
CounterClass = Class.extend({
	...
	set_tint : function (tint) {
		for(var i = 0; i < this.char_sprites.length; i++) {
			...
			this.char_sprites[i].pixisprite.tint = tint;
	...
	update_pos : function (x_start,y_start,w,h) {
		...
		if (this.str_len == 2) x_start = x_start - 14;
		...
		for(var i = 0; i < this.char_sprites.length; i++) {
			...
			this.char_sprites[i].update_pos(x,y);
			...
			if (i < this.char_sprites.length - 1) x = x + 22;//this.font_size;
...
TextClass = Class.extend({
	...
	set_scale: function (scale) {
		if (using_phaser == true) {
			...
			this.pixitext.scale.x = scale;
			this.pixitext.scale.y = scale;
		}
	},
	...
	set_text : function(str) {
		...
		if (using_phaser == true) {
			...
			var thick_ = 4;
			if (this.font_type == Types.Fonts.LARGE) thick_ = 6;
			this.style = {	
				font : font,				
				fill : this.fill,	
...
LayerClass = Class.extend({
	...
	make_particlecontainer : function () {
		if (using_phaser == true) return;

		this.particlecontainer = true;
		this.layer = new PIXI.ParticleContainer(400, {uvs:true, scale:true, rotation: true}, 400);
	},
	...
	filter_on : function () {
		if (using_phaser == true) return;
		if (this.filter_obj == null) {
			this.filter_obj = new new PIXI.filters.BlurYFilter();
		}
		this.layer.filters = [this.filter_obj];
	},
...
SpriteClass = Class.extend({
	...
	setup_sprite: function(name,layer,x,y) {
		...
		if (using_phaser == true) {
			...
			if (layer == Types.Layer.GAME) game_group.add(this.pixisprite);		// game_container
			else if(layer == Types.Layer.GAME_EFFECTS) game_effects_group.add(this.pixisprite);   // options_menu_container
			else if(layer == Types.Layer.POP_MENU) options_menu_group.add(this.pixisprite);   // options_menu_container
			else if(layer == Types.Layer.GAME_MENU) game_menu_group.add(this.pixisprite);  // game_menu_container
			else if(layer == Types.Layer.HUD) menu_group.add(this.pixisprite);	// menu_container
			else if(layer == Types.Layer.TILE) tile_group.add(this.pixisprite);	// tile_container
			...
			else if(this.layer == Types.Layer.TILE_BG) tilebackground_group.add(this.pixisprite);
			else if(this.layer == Types.Layer.TILE_TOP) tile_top_group.add(this.pixisprite);
			else if(this.layer == Types.Layer.TILE_MID) tile_mid_group.add(this.pixisprite);
			else if(this.layer == Types.Layer.TILE_SHADOW) tile_shadow_group.add(this.pixisprite);

			if (layer == Types.Layer.GAME) this.layer_obj = game_group;
			else if(layer == Types.Layer.GAME_EFFECTS) this.layer_obj = game_effects_group;
			else if(layer == Types.Layer.POP_MENU) this.layer_obj = options_menu_group;
			else if(layer == Types.Layer.GAME_MENU) this.layer_obj =game_menu_group;
			else if(layer == Types.Layer.HUD) this.layer_obj =menu_group;
			else if(layer == Types.Layer.TILE) this.layer_obj =tile_group;
			else if(layer == Types.Layer.BACKGROUND) this.layer_obj =background_group;
			else if(this.layer == Types.Layer.GAME_SCREEN) this.layer_obj =game_screen_group;
			else if(this.layer == Types.Layer.LEVEL_TEXT) this.layer_obj =text_group;
			else if(this.layer == Types.Layer.STATIC_TILE) this.layer_obj =static_tile_group;
			else if(this.layer == Types.Layer.TILE_BG) this.layer_obj =tilebackground_group;
			else if(this.layer == Types.Layer.TILE_TOP) this.layer_obj =tile_top_group;
			else if(this.layer == Types.Layer.TILE_MID) this.layer_obj =tile_mid_group;
			else if(this.layer == Types.Layer.TILE_SHADOW) this.layer_obj =tile_shadow_group;

			if (this.layer_obj != null &&
			    this.layer_obj.particlecontainer == true) this.in_particle_container = true;
		}
	...
	make_vis: function() {
		...
		if (using_phaser == true) {
			...
			if (this.in_particle_container == true) {
				this.pixisprite.position.x = this.x;
				this.pixisprite.position.y = this.y;
	...
	set_scale: function(xscale, yscale) {
		...
		this.pixisprite.scale.x = xscale;

		this.pixisprite.scale.y = yscale;
	...
	update_pos: function(x,y) {
		...
		if (using_phaser == true) {
			...
			this.pixisprite.position.x = Math.floor(x);
			this.pixisprite.position.y = Math.floor(y);
	...
	set_pivot: function (xa, ya) {
		this.pixisprite.pivot.set(xa, ya);
	},
	...
	set_texture: function(name) {
		...
		if (using_phaser == true) {
			...
			if (this.keep_rotation == false) this.pixisprite.rotation = 0;
...
var tilebackground_group;
var tile_group;
var tile_top_group;
var tile_mid_group;
var tile_shadow_group;
var game_group;
var game_effects_group;
var menu_group;
var game_menu_group;
...
var tilebackground_container;
var tile_container;
var tile_top_container;
var tile_mid_container;
var tile_shadow_container;
var game_container;
var game_effects_container;
var menu_container;
...
setup_layers = function () {
	//console.log("using particle container for tile + game layers");
	background_group = new LayerClass();//game.add.group();
	tilebackground_group = new LayerClass();
		static_tile_group = new LayerClass();//game.add.group();
		tile_group = new LayerClass();//game.add.group();
		//tile_group.make_particlecontainer();
		game_group = new LayerClass();//game.add.group();
		//game_group.make_particlecontainer();
		game_effects_group = new LayerClass();//game.add.group();
		menu_group = new LayerClass();//game.add.group();

		tile_top_group = new LayerClass();
		tile_mid_group = new LayerClass();
		tile_shadow_group = new LayerClass();
		...
		play_screen_group.add(tile_shadow_group.layer);
		play_screen_group.add(tile_mid_group.layer);
		play_screen_group.add(tile_top_group.layer);
		play_screen_group.add(game_group.layer);
		play_screen_group.add(game_effects_group.layer);
		
		play_group.add(background_group.layer);
		play_group.add(tilebackground_group.layer);
		...
		tile_top_container = tile_top_group ;
		tile_mid_container = tile_mid_group; 
		tile_shadow_container = tile_shadow_group;
