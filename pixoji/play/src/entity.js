...
OverworldSpritesClass = Class.extend({
	...
	add_level: function (spritename) {
		...
		var box_ = new TableAreaClass(this.game_state, "");
	...
	hide: function() {
		for (var i = 0; i < this.level_x.length; i++) {
			...
			this.level_box[i].resize(-999,-999,
			     		 	 -999,-999);
  	...
  	make_vis: function() {
		for (var i = 0; i < this.level_x.length; i++) {
			...
			this.level_box[i].resize(x - 45, y - 45,
			     		 	 x + 45, y + 45);
...
BlockClass = Class.extend({
	...
	init: function(game_state) {
		...
		// "flagblock.png"  "eye.png"   "hand.png"   "8hand.png"    "eyeplustouch.png"

		this.block_sprite = new SpriteClass();
		this.block_sprite.setup_sprite(g_block_sprites[this.block_type],Types.Layer.TILE);
